...
class SNFData():
    ...
    def __init__(self, Nmodes, measurement_radius, folder, files, name, fake = False, duplicates = 'overwrite', phiCut = (10, 2.5), phiAdjust = [], gravitationalDroop = (0, 0), gravDroopDist = 10e-2, phiAdjustPlotting = 0):
        ...
        :param name: String to refer to this dataset
        :param fake: I previously used this to insert synthetic data... this is now deprecated, should always be False
        :param duplicates: What to do with duplicate data points - default to simply 'overwrite'. Can also 'average' and 'ignore'
        :param phiCut: For (a, b) remove the data for the first a and the last b degrees. This is to compensate for the hysterisis in the rotator + possible acceleration effects
        :param phiAdjust: A list of two arrays of (theta angle, phi angle adjustment) for use aligning phi angles, one list for each probe pol. [[pp=0], [pp=90]]. This is to compensate for the phi-rotator drifting between sweeps.
        ...
        if(fake): ## if fake, pass in the fake data as 'files'. Currently the code has no fake data to try, so this is unnecessary
            ...
            for i in range(len(files)): ## data is in the Format: # of triggers, time since start, probe pol, theta angle [degrees], phi angle, f1 [Hz], S21(f1), f2, S21(f2), f3, S21(f3)
                ...
                self.numMeas += np.shape(load)[0]
                
                ## some plots and plot setup to determine phi adjustments
                if(phiAdjustPlotting%2 == 1):## find the phi-angle misalignment for each data file
                    if(np.isclose(load[0, 2], 90)): ## then pol 1
                        self.plotContoursPreProcessing(load, pol=1) ### this line makes a contour plot of the raw data before any processing
                    else:
                        self.plotContoursPreProcessing(load, pol=0) ### this line makes a contour plot of the raw data before any processing
                
                thets = np.round(np.real(load[:,3]), 3) ## pre-adjustment theta points
                ...
                if(len(phiAdjust) > 0 ): ## try this adjustment
                    phijust = phiAdjust[i]
                    ## order then in an increasing sequence:
                    idx = np.argsort(phijust[0])
                    phijust = phijust [:, idx]
                    ...
                    uniquethetas = np.sort(np.real(np.unique(load[:,3])))
                    smoothingFrac = 1/8 ## more for more smoothness
                    ##
                    
                    y_makimapp0 = scipy.interpolate.Akima1DInterpolator(phijust[0], phijust[1], method = 'makima') ## ultimate values to adjust
                    ...
                    if(phiAdjustPlotting > 1):
                        plt.title('Phi Adjustments by Theta Angle')
                        plt.xlabel('Theta [degrees]')
                        plt.ylabel('Phi [degrees]')
                        alph2 = .2
                        if(i == 0): ## give labels
                            plt.plot(phijust[0], phijust[1], label=f'Raw Est. Adjustments, pp={np.real(load[0,2]):.0f}, '+'$\phi_\mathrm{start}='+f'${np.real(load[0, 4]):.0f}', linestyle = '--', linewidth = 2.2, color = colors[i])
                            plt.plot(uniquethetas, adjusterpp0, linewidth = 1.9, linestyle = ':', alpha = alph2, color = colors[i], label='Interpolation')
                            plt.plot(uniquethetas, roundadjustpp0, alpha = alph2, color = colors[i], label='Interpolation, rounded')
                        else:
                            plt.plot(phijust[0], phijust[1], label=f'pp={np.real(load[0,2]):.0f}, '+'$\phi_\mathrm{start}='+f'${np.real(load[0, 4]):.0f}', linestyle = '--', linewidth = 2.2, color = colors[i])
                            plt.plot(uniquethetas, adjusterpp0, linewidth = 1.9, linestyle = ':', alpha = alph2, color = colors[i])#, label='pp=0 Interpolation')
                            plt.plot(uniquethetas, roundadjustpp0, alpha = alph2, color = colors[i])#, label='pp=0 Interpolation, rounded')
                        ## then plot it
                        plt.legend()
                        plt.tight_layout()
                    
                    ### now adjust phi angles
                    load[:, 4] = load[:, 4] + np.interp(np.real(load[:, 3]), phijust[0], phijust[1])
                    load = SNFAMFunctions.interpToGrid(load, thets, phs) # Then need to interpolate points back onto the regular grid, for later analysis
                    ## if not interping here, the plots are not... proper
                    
                    if(phiAdjustPlotting%2 == 1): ## now plot post-adjustment, pre-cutting
                        if(np.isclose(load[0, 2], 90)): ## then pol 1
                            self.plotContoursPreProcessing(load, pol=1) ###contour plot of the data again after adjusting phi angles
                        else:
                            self.plotContoursPreProcessing(load, pol=0) ###contour plot of the data again after adjusting phi angles
                            
                    ## now cut out all phi-vals that I longer have all theta-points for, or are too close to the start/end (this should be the same for all pols, so do the same for each datafile):
                    maxPhAdj = 0
                    minPhAdj = 0
                    for op in range(len(files)): ## quickly calc the max and mins.. list of arrays, so not sure how to better do this
                        maxPhAdj = np.max((np.max(phiAdjust[op][1]), maxPhAdj))
                        minPhAdj = np.max((np.max(phiAdjust[op][1]), minPhAdj))
                    phiMin = np.min(phs) + phiCut[0] + maxPhAdj ## max. phi adjustment plus minmum accepted value
                    phiMax = np.max(phs) - phiCut[1] - minPhAdj ## maximum accepted value plus min. phi adjustment
                    if(i==0): ## just say it once
                        print(f'Cutting phi values to be between {phiMin} and {phiMax}')
                    load = load[(load[:,4]>(phiMin))]
                    load = load[(load[:,4]<(phiMax))]            
                    
                    #######
                    #### PHI ADJUSTMENT ENDS
                    #######
                
                    if(phiAdjustPlotting%2 == 1): ## now plot post-cutting
                        if(np.isclose(load[0, 2], 90)): ## then pol 1
                            self.plotContoursPreProcessing(load, pol=1) ###contour plot of the data again after adjusting phi angles
                        else:
                            self.plotContoursPreProcessing(load, pol=0) ###contour plot of the data again after adjusting phi angles
                ##
                ## GRAV DROOP STARTS - now do gravitational droop stuff - this clashes with the phi adjustment and the phi-angle changes at low theta make this difficult anyway, so this is not done.
                ##
                if(gravitationalDroop[0] > 0 or gravitationalDroop[1] > 0): ## Rotate each data point to compensate for gravitational droop
                    uniquephis = np.sort(np.real(np.unique(load[:,4])))
                    ## try alphas as a sine fit
                    ## max alpha at phi = 90 =>
                    alphas = gravitationalDroop[1] + np.abs(np.sin(uniquephis*pi/180))*(gravitationalDroop[0]-gravitationalDroop[1]) ## in degrees
                    dats = SNFAMFunctions.gravityDroopCompensation(load, alphas*pi/180, gravDroopDist, measurement_radius, doPlots = phiAdjustPlotting)
                    load = SNFAMFunctions.interpToGrid(load, thets, phs) # Then need to interpolate points back onto the regular grid, for later analysis
                    if(phiAdjustPlotting > 2): ## plot some diagnostics
                        plt.plot(uniquephis, alphas)
                        plt.show()
                        ...
                        if(np.isclose(load[0, 2], 90)): ## then pol 1
                            ...
                            self.plotContoursPreProcessing(load, pol=0) ###contour plot of the data again after adjusting phi angles
                    self.plotContoursPreProcessing(load, pol=1)
                ### GRAV DROOP ENDS
                ##
        ...
        print(self.name+f' loaded, with {self.len} data-points ({self.numMeas} meas. points)')
        ...
        for n in range(self.len): ## each data point
            ...
            if(phi < 0): ## in case of measurements at negative phi angles... I haven't gone below -180, so this should work fine
                phi = phi+360
        ...
        ## For any values at theta=0 for which we have no phi values, take a phi value. It will be 'flipped', so phase-shift of -1
        for a in range(len(self.phivec_sphere)): # each phi val
            phival = self.phivec_sphere[a]
            for pp in [0, 1]: # each probe pol
                itheta = np.argwhere(np.abs(self.thetavec_sphere) == 0)[0] ## index of theta = 0 for this phi and probe pol
                if(self.S21_sphere[0, pp, itheta, a] == 0): ## just check first freq. to see if there is a value or it is still zero
                    if( (phival+180)<self.phiRange[-1] ): # check above
                        iphi = np.argmin(np.abs(self.phivec_sphere - (phival+180))) # index of the phi value above
    ...
    def plotFarField(self, normalize = True, levels = None, Nlevels = 45, pol = 2, spacing = 1, Nplot = 777, freq = 1, plotCuts = False, plotContours = True, colourbar = False): ## uses calculated transmission coefficients to calculate the far-field. Creates data on the sphere as in plot(), then plots planes (TODO)
        ...
        if(plotCuts):
            ...
            if(normalize): ## different value needed for plotPatterns
                normalize = 'together'
            ###
            ### THETA CUTS
            ###
            ...
            phisE[thetas<0] += pi
            phisH[thetas<0] += pi
            thetavec = np.abs(thetas)*pi/180
            Eplane = SNFAMFunctions.findFarField(self.Ts, thetavec, phisE)
            Hplane = SNFAMFunctions.findFarField(self.Ts, thetavec, phisH)
            
            if(pol==0):
                Es = 20*np.log10(np.sqrt(np.abs(Eplane[0])**2))
                Hs = 20*np.log10(np.sqrt(np.abs(Hplane[0])**2))
                titleAdd = ', Theta-pol (Vert.)'
            elif(pol==1):
                Es = 20*np.log10(np.sqrt(np.abs(Eplane[1])**2))
                Hs = 20*np.log10(np.sqrt(np.abs(Hplane[1])**2))
                titleAdd = ', Phi-pol (Horiz.)'
            elif(pol==2):
                Es = 20*np.log10(np.sqrt(np.abs(Eplane[0])**2 + np.abs(Eplane[1])**2))
                Hs = 20*np.log10(np.sqrt(np.abs(Hplane[0])**2 + np.abs(Hplane[1])**2))
                titleAdd = ', Magnitude'
            
            
            if(normalize):
                maxval = np.max(np.vstack((Es, Hs)))
                Es -= maxval
                Hs -= maxval
            ...
            ax.set_xticks(np.linspace(180, -180, 24, endpoint=False)*pi/180)
    ...
    def plotContoursPreProcessing(self, data, levels = None, Nlevels = 32, pol = 2, phase = 0, FF = False, normalize=False): ### plots the unprocessed data
        ...
        ## set any leftover zeros to just very close to zero 
        idx0 = np.isclose(S21s, 0, atol=1e-1) ## atol chosen by necessity... presumably any real measurement has much lower levels than ~~0
    ...
    def plotContours(self, Ss, freq = 1, levels = None, Nlevels = 22, pol = 2, phase = 0, FF = False): ### plots the data
        ...
        plt.ylabel('phi (degrees)')
        if(FF):
            plt.title(f'{self.name} FF, frequency {self.fs[freq]*1e-9:.4} GHz')
        else:
...
def plotPlanes(datas, fs = [1], pPs = []): ## plots the phi = 0 and phi = 90 planes for both probe pols. Can send in phase factors for each dataset. This cannot be trusted, because it searches for the nearest data - which may not exist
        val1 = 0 ## compare between axis 2 = val1 and axis 2 = phi2
...
def plotPatterns(patterns, normalize = 'each', cuts=[0, 90], axes = [r'$\theta$', r'$\phi$']): ## takes a list of patterns given by an array of intensities at e.g. theta angles along phi cuts, the theta values, and a name [[[I_phi=0, thetas],[I_phi=90, thetas],'name'],...]
    ...
    for pattern in patterns:
        ...
        # first sort the data by angle, in case it comes in unsorted
        idx1 = np.argsort(pattern[0][1])  
        idx2 = np.argsort(pattern[1][1])    
        
        ax1a.plot(pattern[0][1][idx1]*180/pi, 20*np.log10(pattern[0][0][idx1]*normFactor), linestyle = linestyles[c%len(linestyles)], color = colors[c%len(linestyles)], label = pattern[2]) #1-cut
        ax1b.plot(pattern[1][1][idx2]*180/pi, 20*np.log10(pattern[1][0][idx2]*normFactor), linestyle = linestyles[c%len(linestyles)], color = colors[c%len(linestyles)], label = pattern[2]) #2-cut
        c+=1
...
#===============================================================================
# files = ['9.35GHzArray+0.2lambdaspacedcloakedsLaterTryPol0.csv'] ## slot array with evenly .2lambda at 1.045 GHz-spaced uncloaked antennas on top. first positioned in mid. This is when the motor broke, so not finished.
...
# DRH50HornProbeAdjust.append(np.array([[-50, -35, -23, -10, 0, 5, 11, 20, 27, 41.8, 48, 58, 72], [-6, -6, -5, -4, -3.5, -3, -3.4, -3, -2, -1.8, -1.2, -.6, -.5]])) ## pp=0, adjust to have 0 occur at phi=90
# DRH50HornProbeAdjust.append(np.array([[-42, -35, -25, -12, 0, 16, 23, 35.5, 44.7, 72], [6.5, 5.6, 5.6, 6, 7.3, 8.6, 8.2, 9, 10, 12.5]])) ## pp=90, adjust for 0 at phi = 180
# p2p = SNFData(J, measDist, folder, files, name = 'ProbesMeas', phiAdjust = DRH50HornProbeAdjust, phiAdjustPlotting = 0)
